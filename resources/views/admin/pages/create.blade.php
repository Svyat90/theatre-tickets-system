...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ __('global.create') }} {{ __('cruds.pages.title_singular') }}
        </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.pages.store") }}">
                ...
                <div class="row">
                                                ...
                                                @if ($field === 'description' || $field === 'content')
                                                    <div class="form-group">
                                                        <label for="{{ $name = $field . '[' . $language->locale . ']' }}">
                                                            {{ trans("cruds.pages.fields.$field") }}
                                                        </label>
                                                        ...
                                                        @endif
                                                        <span class="help-block">{{ trans("cruds.pages.fields.{$field}_helper") }}</span>
                                                    </div>
                                                @else
                    ...
                    <div class="form-group col-md-4 col-sm-4 col-xs-4">
                        ...
                        <select class="form-control {{ $errors->has($name) ? 'is-invalid' : '' }}"
                            ...
                            @foreach($pages as $id => $page)
                                <option value="{{ $id }}" {{ $id == old($name, 0) ? 'selected' : '' }}>{{ $page }}</option>
                            @endforeach
                        </select>
                        @if($errors->has($name))
                            <span class="text-danger">{{ $errors->first($name) }}</span>
                    ...
                    <div class="form-group col-md-4 col-sm-4 col-xs-4">
                        <label class="" for="{{ $name = 'footer_column' }}">{{ __('global.footer_column') }}</label>
                        <select class="form-control {{ $errors->has($name) ? 'is-invalid' : '' }}"
                                name="{{ $name }}"
                                id="{{ $name }}" >
                            <option>{{ trans('global.pleaseSelect') }}</option>
                            @foreach(['column_1' => 'column_1', 'column_2' => 'column_2', 'column_3' => 'column_3' ] as $key => $column)
                                <option value="{{ $key }}" {{ $id == old($name, 0) ? 'selected' : '' }}>{{ $column }}</option>
                            @endforeach
                        </select>
                        @if($errors->has($name))
                            <span class="text-danger">{{ $errors->first($name) }}</span>
