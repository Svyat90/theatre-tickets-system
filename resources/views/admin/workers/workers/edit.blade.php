...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ __('global.edit') }} {{ __('cruds.workers.title_singular') }}
        </div>
                                        ...
                                        @foreach($worker->getTranslatable() as $field)
                                            ...
                                            <div class="form-group col-md-12 col-sm-12 col-xs-12">
                                                ...
                                                <input class="form-control {{ $errors->has($name) ? 'is-invalid' : '' }}"
                                                       ...
                                                       name="{{ $name }}"
                                                       id="{{ $name }}"
                                                       value="{{ $oldLocale[$language->locale] ?? columnTrans($worker, $field, $language->locale) }}" />
                                                @if($errors->has($name))
                    ...
                    <div class="form-group col-md-4 col-sm-4 col-xs-4">
                        ...
                        <select class="form-control {{ $errors->has($name) ? 'is-invalid' : '' }}"
                            ...
                            @foreach($workerCategories as $id => $workerCategory)
                                <option value="{{ $id }}" {{ $id == old($name, $worker->category->id ?? 0) ? 'selected' : '' }}>{{ $workerCategory }}</option>
                            @endforeach
                        </select>
                        @if($errors->has($name))
                            <span class="text-danger">{{ $errors->first($name) }}</span>
...
@section('scripts')
    <script>
        let imageGridDropZone = new Dropzone("#image", {
            ...
            init: function () {
                @php $media = $worker->getFirstMedia($name = 'image'); @endphp
                @if($media)
